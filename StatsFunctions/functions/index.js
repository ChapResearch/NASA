...
exports.statistics = functions.database.ref('{year}/{robot}/{competition}/{match}').
	...
	// it back to the database
	    ...
	    .then((xmldata) => calcMetaData(snapshot.val(),context.params,xmldata))
...
function calcMetaData(data,params,xmldata)
{
	...
	data[metaData[i].name] = calcMetaDataField(data,metaData[i]);
    }
...
//
function calcMetaDataField(data,field)
{
}
